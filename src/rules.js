...
const READ_ONLY_PATH_TOOLS = /* @__PURE__ */ new Set(["read", "ls", "list", "tree", "find", "grep", "rg", "search"]);
const PROTECTED_SKILL_HELPER_INTERPRETERS = /* @__PURE__ */ new Set([
  "node",
  "nodejs",
  "python",
  "python3",
...
function isReadOnlyShellCommand(command) {
  ...
  if (first === "find") {
    return !tokens.some((token) => ["-delete", "-exec", "-execdir", "-ok", "-okdir"].includes(token.toLowerCase()));
  }
  if (first === "sed") {
    return tokens.includes("-n") && !tokens.some((token) => {
...
function isReadOnlyProtectedPathAccess(toolName, params) {
...
function isAllowedProtectedSkillHelperExecution(toolName, params, allowedRoots, baseDir = process.cwd()) {
  const normalizedTool = normalizeToolName(toolName);
  ...
  if (pathCandidates.length === 0 || !pathCandidates.every((candidate) => isPathWithinAnyRoot(candidate, allowedRoots))) {
    return false;
  }
  const first = tokens[0]?.toLowerCase();
  if (!first) {
...
function resolveProtectedPathViolation(toolName, params, protectedRoots, baseDir = process.cwd(), options) {
  ...
  if (readOnlyAllowedRoots.length > 0 && (isReadOnlyProtectedPathAccess(toolName, params) || isAllowedProtectedSkillHelperExecution(toolName, params, readOnlyAllowedRoots, baseDir)) && matches.every((candidate) => isPathWithinAnyRoot(candidate, readOnlyAllowedRoots))) {
    return { blocked: false, matches: [] };
  }
  return {
    blocked: true,
...
function resolveInlineExecutionViolation(command, protectedRoots, baseDir = process.cwd(), options) {
  if (!command) {
    return void 0;
  }
  ...
  if (tokens[1]) {
    ...
    if (matchesProtectedPathTarget(scriptCandidate, protectedRoots)) {
      ...
      if (allowedExecutionRoots.length > 0 && isAllowedProtectedSkillHelperExecution("exec", { command }, allowedExecutionRoots, baseDir)) {
        return void 0;
      }
      return BLOCK_REASON_PROTECTED_PATH;
    }
