...
const PREFIXED_API_KEY_PATTERNS: ReadonlyArray<{
  category: "api_key" | "secret_token";
  placeholder: string;
  regex: RegExp;
}> = [
...
const HIGH_ENTROPY_TOKEN_RE =
  /(?<![A-Za-z0-9\-_.~+/=])[A-Za-z0-9\-_.~+/]{20,}={0,3}(?![A-Za-z0-9\-_.~+/=])/g;
...
export type PromptSanitizationRiskFlag =
  | "llm-prompt-email-address"
  | "llm-prompt-phone-number"
  | "llm-prompt-api-key"
  | "llm-prompt-secret-token"
  | "llm-prompt-encoded-secret";

export type PromptSanitizationOutcome = {
  value: string;
  changed: boolean;
  redactionCount: number;
  matchedCategories: string[];
  categoryCounts: Record<string, number>;
  riskFlags: PromptSanitizationRiskFlag[];
...
function appendUnique(values: string[], nextValues: Iterable<string>): string[] {
...
function uniqueSortedSecretCandidates(values: string[]): string[] {
...
export function sanitizeLlmPromptText(
...
): PromptSanitizationOutcome {
  const categoryCounts = new Map<string, number>();
  const riskFlags = new Set<PromptSanitizationRiskFlag>();
  let secretCandidates: string[] = appendUnique([], options.observedSecrets ?? []);
  let next = text;

  next = next.replace(EMAIL_ADDRESS_RE, (match) => {
    incrementCount(categoryCounts, "email_address");
    riskFlags.add("llm-prompt-email-address");
    return "<EMAIL_ADDRESS>";
  });

  next = next.replace(GENERIC_PHONE_RE, (match) => {
    incrementCount(categoryCounts, "phone_number");
    riskFlags.add("llm-prompt-phone-number");
    return "<PHONE_NUMBER>";
  });

  next = next.replace(CN_MOBILE_PHONE_RE, (match) => {
    incrementCount(categoryCounts, "phone_number");
    riskFlags.add("llm-prompt-phone-number");
    return "<PHONE_NUMBER>";
  });

  next = next.replace(LABELED_API_KEY_RE, (_match, prefix: string, value: string, suffix: string) => {
  ...
  next = next.replace(LABELED_SECRET_RE, (_match, prefix: string, value: string, suffix: string) => {
