...
const MANAGED_OVERLAY_RECONCILE_MODES = ["off", "observe", "enforce"];
...
function deriveManagedOverlayId(authorityPath, livePath) {
...
function isManagedOverlayReconcileMode(value) {
  return typeof value === "string" && MANAGED_OVERLAY_RECONCILE_MODES.includes(value);
}

function normalizeManagedOverlayConfigEntries(value, resolvePath = (input) => input) {
  if (!Array.isArray(value)) {
    return [];
  }
  const byId = new Map();
  const seenLivePaths = new Set();
  for (const entry of value) {
    ...
    const reconcileMode = isManagedOverlayReconcileMode(raw.reconcileMode) ? raw.reconcileMode : isManagedOverlayReconcileMode(raw.mode) ? raw.mode : "enforce";
...
function collectManagedOverlayProtectedRoots(overlays) {
  const roots = new Set();
...
async function copyAuthorityToLive(authorityPath, livePath) {
...
async function tryReadFingerprint(filePath) {
  try {
...
async function persistManagedOverlayStateFile(filePath, records) {
...
async function reconcileManagedOverlays(params) {
  ...
  for (const overlay of params.overlays) {
    ...
    try {
      ...
      record.lastAction = error instanceof Error && error.message.includes("ENOENT") ? "authority_missing" : "error";
...
export {
  MANAGED_OVERLAY_RECONCILE_MODES,
